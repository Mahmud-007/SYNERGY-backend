...
router.put('/signup', [
    body('username')
        ...
        .custom((value, {req}) => {
            return People.findOne({username: value})
                .then(user => {
                    if (user) {
    ...
    body('email')
        ...
        .custom((value, {req}) => {
            return People.findOne({email: value})
                .then(user => {
                    if (user) {
                        return Promise.reject('Account already exist with the E-Mail.');
                    }
                });
        })
        .normalizeEmail(),
    ...
    body('email')
        ...
        .custom((value, {req}) => {
            return People.findOne({email: value})
                .then(user => {
                    if (!user) {
                        return Promise.reject('Account not found with the email');
                    }
                });
        })
        .normalizeEmail(),
